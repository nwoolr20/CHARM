 ...
 * 
 * This is the normative reference implementation of the CHARM algorithm.
 * All optimized implementations must produce bit-for-bit identical results.
 * 
 * Version: CHARM/1.0
...
int main(int argc, char* argv[]) {
    if (argc > 1 && strcmp(argv[1], "--version") == 0) {
        printf("CHARM Reference Implementation %s\n", charm_version());
        return 0;
    }
    
    printf("CHARM Reference Implementation - Basic Test\n");
    
    // Test empty string
    uint8_t digest[32];
    int ret = charm_hash(CHARM_256, "", 0, digest);
    if (ret != 0) {
        printf("Error: charm_hash failed with code %d\n", ret);
        return 1;
    }
    ...
    // Test "abc"
    ret = charm_hash(CHARM_256, "abc", 3, digest);
    if (ret != 0) {
        printf("Error: charm_hash failed with code %d\n", ret);
        return 1;
    }
    
    printf("CHARM-256(\"abc\") = ");
    for (int i = 0; i < 32; i++) {
        printf("%02x", digest[i]);
    }
    printf("\n");
    
    printf("Reference implementation test completed successfully.\n");
    return 0;
}